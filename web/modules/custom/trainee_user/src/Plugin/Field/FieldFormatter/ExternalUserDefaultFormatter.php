 ...
 *
 * @FieldFormatter(
 *   id = "external_user_default_formatter",
 *   label = @Translation("User info(name and email)"),
 *   field_types = {
 *     "user_reference_field"
 *   }
 * )
 */
class ExternalUserDefaultFormatter extends FormatterBase {
  ...
  /**
   * Constructs a new ExternalUserDefaultFormatter.
   *
  ...
  public function viewElements(FieldItemListInterface $items, $langcode): array {
    ...
    foreach ($items as $delta => $item) {
      if (isset($item->external_user_id)) {
        ...
        $element[$delta] = [
          '#type' => 'markup',
          '#markup' => "Name: {$user['name']}</br> Email: {$user['email']}",
        ];
      }
    }
